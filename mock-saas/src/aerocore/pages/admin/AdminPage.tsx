...
import { Plus, Search, Shield, Trash2, Edit, RefreshCw, Lock, CheckSquare, Square, Key, Activity, Server, Database, Wifi, Settings, Mail } from 'lucide-react';
...
export function AdminPage() {
  ...
  const systemMetrics = {
      ...
      load: [45, 52, 48, 60, 55, 67, 70, 65, 50, 45, 42, 48]
  };
        ...
        {activeTab === 'users' && (
            <>
                ...
                {selectedUsers.length > 0 && (
                    <div className="bg-slate-800/50 border border-slate-700 p-3 rounded-lg flex items-center justify-between animate-in slide-in-from-top-2 fade-in">
                        <div className="text-sm text-slate-300 font-medium px-2">
                            <span className="text-white">{selectedUsers.length}</span> users selected
                        </div>
                        <div className="flex items-center gap-2">
                            <button 
                                onClick={handleBulkDeactivate}
                                className="px-3 py-1.5 bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 border border-rose-500/20 rounded text-xs font-medium flex items-center gap-2 transition-colors"
                            >
                                <Trash2 size={14} /> Deactivate Selected
                            </button>
                            <button 
                                onClick={handleBulkEmail}
                                className="px-3 py-1.5 bg-sky-500/10 hover:bg-sky-500/20 text-sky-400 border border-sky-500/20 rounded text-xs font-medium flex items-center gap-2 transition-colors"
                            >
                    ...
                    <table className="w-full text-sm text-left">
                        <thead className="bg-slate-950 text-slate-400 font-medium border-b border-slate-800">
                            <tr>
                                <th className="px-4 py-3 w-10">
                                    <button onClick={toggleSelectAll} className="flex items-center text-slate-500 hover:text-white">
                                        {selectedUsers.length === state.users.length && state.users.length > 0 ? <CheckSquare size={16} /> : <Square size={16} />}
                                    </button>
                                </th>
                        ...
                        <tbody className="divide-y divide-slate-800">
                            {state.users.map(user => (
                                <tr key={user.id} className={`hover:bg-slate-800/50 group transition-colors ${user.status === 'Inactive' ? 'opacity-50 grayscale-[0.5]' : ''} ${selectedUsers.includes(user.id) ? 'bg-slate-800/80' : ''}`}>
                                    <td className="px-4 py-3">
                                        <button onClick={() => toggleSelectUser(user.id)} className={`flex items-center ${selectedUsers.includes(user.id) ? 'text-sky-500' : 'text-slate-600 hover:text-slate-400'}`}>
                                            {selectedUsers.includes(user.id) ? <CheckSquare size={16} /> : <Square size={16} />}
                                        </button>
                                    </td>
                                    <td className="px-4 py-3">
                                        <div>
