import { C as ChatOpenAI, S as SystemMessage, H as HumanMessage } from "./main-DpNFm6Qd.js";
import { domContextService } from "./DOMContextService-Cp2lSEzr.js";
import { app } from "electron";
import path__default from "node:path";
import fs__default from "node:fs";
function getApiKey() {
  if (process.env.NVIDIA_API_KEY) {
    return { key: process.env.NVIDIA_API_KEY, isNvidia: true };
  }
  if (process.env.OPENAI_API_KEY) {
    return { key: process.env.OPENAI_API_KEY, isNvidia: false };
  }
  const possiblePaths = [
    path__default.join(process.cwd(), ".env"),
    path__default.join(app.getAppPath(), ".env"),
    path__default.join(app.getAppPath(), "..", ".env")
  ];
  for (const envPath of possiblePaths) {
    try {
      if (fs__default.existsSync(envPath)) {
        const content = fs__default.readFileSync(envPath, "utf8");
        const nvidiaMatch = content.match(/^NVIDIA_API_KEY=(.+)$/m);
        if (nvidiaMatch) {
          return { key: nvidiaMatch[1].trim(), isNvidia: true };
        }
        const openaiMatch = content.match(/^OPENAI_API_KEY=(.+)$/m);
        if (openaiMatch) {
          return { key: openaiMatch[1].trim(), isNvidia: false };
        }
      }
    } catch {
    }
...
class CodeGeneratorService {
  ...
  getModel(options = {}) {
    ...
    const baseUrl = apiKeyInfo.isNvidia ? "https://integrate.api.nvidia.com/v1" : void 0;
    const modelName = apiKeyInfo.isNvidia ? "meta/llama-3.3-70b-instruct" : "gpt-4o-mini";
   ...
   */
  async *generateStream(command, context, options = {}) {
    try {
      const domContext = context ?? await domContextService.getContext().catch(() => null);
      const userPrompt = this.buildUserPrompt(command, domContext, options);
      const model = this.getModel(options);
