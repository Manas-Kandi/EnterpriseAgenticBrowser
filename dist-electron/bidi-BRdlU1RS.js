...
class BrowserProcessor {
  ...
  async createUserContext(params) {
    ...
    if (w3cParams.acceptInsecureCerts !== void 0) {
      if (w3cParams.acceptInsecureCerts === false && globalConfig.acceptInsecureCerts === true)
        throw new protocol_js_1$r.UnknownErrorException(`Cannot set user context's "acceptInsecureCerts" to false, when a capability "acceptInsecureCerts" is set to true`);
    }
    const request = {};
    if (w3cParams.proxy) {
  ...
  async removeUserContext(params) {
    ...
    if (userContext === "default") {
      throw new protocol_js_1$r.InvalidArgumentException("`default` user context cannot be removed");
    }
    try {
      ...
      if (err.message.startsWith("Failed to find context with id")) {
        throw new protocol_js_1$r.NoSuchUserContextException(err.message);
      }
      throw err;
    }
    return {};
...
toCdpDownloadBehavior_fn = function(downloadBehavior) {
  ...
  throw new protocol_js_1$r.UnknownErrorException("Unexpected download behavior");
};
...
function getProxyStr(proxyConfig) {
  ...
  if (proxyConfig.proxyType === "pac") {
    throw new protocol_js_1$r.UnsupportedOperationException(`PAC proxy configuration is not supported per user context`);
  }
  if (proxyConfig.proxyType === "autodetect") {
    throw new protocol_js_1$r.UnsupportedOperationException(`Autodetect proxy is not supported per user context`);
  }
  if (proxyConfig.proxyType === "manual") {
    ...
    if (proxyConfig.socksProxy !== void 0 || proxyConfig.socksVersion !== void 0) {
      if (proxyConfig.socksProxy === void 0) {
        throw new protocol_js_1$r.InvalidArgumentException(`'socksVersion' cannot be set without 'socksProxy'`);
      }
      if (proxyConfig.socksVersion === void 0 || typeof proxyConfig.socksVersion !== "number" || !Number.isInteger(proxyConfig.socksVersion) || proxyConfig.socksVersion < 0 || proxyConfig.socksVersion > 255) {
        throw new protocol_js_1$r.InvalidArgumentException(`'socksVersion' must be between 0 and 255`);
      }
  ...
  throw new protocol_js_1$r.UnknownErrorException(`Unknown proxy type`);
}
var CdpProcessor$1 = {};
...
class CdpProcessor {
  ...
  resolveRealm(params) {
    ...
    if (realm === void 0) {
      throw new protocol_js_1$q.UnknownErrorException(`Could not find realm ${params.realm}`);
    }
...
class BrowsingContextProcessor {
  constructor(browserCdpClient, browsingContextStorage, userContextStorage, contextConfigStorage, eventManager) {
    ...
    __privateGet(this, _eventManager).addSubscribeHook(protocol_js_1$p.ChromiumBidi.BrowsingContext.EventNames.ContextCreated, __privateMethod(this, _BrowsingContextProcessor_instances, onContextCreatedSubscribeHook_fn).bind(this));
  }
  getTree(params) {
  ...
  async create(params) {
    ...
    if (params.referenceContext !== void 0) {
      ...
      if (!referenceContext.isTopLevelContext()) {
        throw new protocol_js_1$p.InvalidArgumentException(`referenceContext should be a top-level context`);
      }
    ...
    try {
      ...
      if (
        ...
        throw new protocol_js_1$p.NoSuchUserContextException(`The context ${userContext} was not found`);
      }
      throw err;
    }
  ...
  async activate(params) {
    ...
    if (!context.isTopLevelContext()) {
      throw new protocol_js_1$p.InvalidArgumentException("Activation is only supported on the top-level context");
    }
  ...
  async setViewport(params) {
    ...
    if ((((_a3 = params.viewport) == null ? void 0 : _a3.height) ?? 0) > maxDimensionSize || (((_b2 = params.viewport) == null ? void 0 : _b2.width) ?? 0) > maxDimensionSize) {
      throw new protocol_js_1$p.UnsupportedOperationException(`Viewport dimension over ${maxDimensionSize} are not supported`);
    }
    const config = {};
  ...
  async traverseHistory(params) {
    const context = __privateGet(this, _browsingContextStorage3).getContext(params.context);
    if (!context) {
      throw new protocol_js_1$p.InvalidArgumentException(`No browsing context with id ${params.context}`);
    }
    if (!context.isTopLevelContext()) {
      throw new protocol_js_1$p.InvalidArgumentException("Traversing history is only supported on the top-level context");
    }
  ...
  async handleUserPrompt(params) {
    ...
    try {
      ...
      if ((_a3 = error.message) == null ? void 0 : _a3.includes("No dialog is showing")) {
        throw new protocol_js_1$p.NoSuchAlertException("No dialog is showing");
      }
      throw error;
    }
    return {};
  ...
  async close(params) {
    const context = __privateGet(this, _browsingContextStorage3).getContext(params.context);
    if (!context.isTopLevelContext()) {
      throw new protocol_js_1$p.InvalidArgumentException(`Non top-level browsing context ${context.id} cannot be closed.`);
    }
...
getRelatedTopLevelBrowsingContexts_fn = async function(browsingContextId, userContextIds) {
  if (browsingContextId === void 0 && userContextIds === void 0) {
    throw new protocol_js_1$p.InvalidArgumentException("Either userContexts or context must be provided");
  }
  if (browsingContextId !== void 0 && userContextIds !== void 0) {
    throw new protocol_js_1$p.InvalidArgumentException("userContexts and context are mutually exclusive");
  }
  if (browsingContextId !== void 0) {
    const context = __privateGet(this, _browsingContextStorage3).getContext(browsingContextId);
    if (!context.isTopLevelContext()) {
      throw new protocol_js_1$p.InvalidArgumentException("Emulating viewport is only supported on the top-level context");
    }
    return [context];
  }
...
onContextCreatedSubscribeHook_fn = function(contextId) {
  ...
  contextsToReport.forEach((context2) => {
    __privateGet(this, _eventManager).registerEvent({
      type: "event",
      method: protocol_js_1$p.ChromiumBidi.BrowsingContext.EventNames.ContextCreated,
...
const ErrorResponse_js_1$3 = ErrorResponse;
class EmulationProcessor {
  ...
  async setGeolocationOverride(params) {
    ...
    if ("coordinates" in params && "error" in params) {
      throw new ErrorResponse_js_1$3.InvalidArgumentException("Coordinates and error cannot be set at the same time");
    }
    ...
    if ("coordinates" in params) {
      if ((((_a3 = params.coordinates) == null ? void 0 : _a3.altitude) ?? null) === null && (((_b2 = params.coordinates) == null ? void 0 : _b2.altitudeAccuracy) ?? null) !== null) {
        throw new ErrorResponse_js_1$3.InvalidArgumentException("Geolocation altitudeAccuracy can be set only with altitude");
      }
      geolocation = params.coordinates;
    } else if ("error" in params) {
      if (params.error.type !== "positionUnavailable") {
        throw new ErrorResponse_js_1$3.InvalidArgumentException(`Unknown geolocation error ${params.error.type}`);
      }
      geolocation = params.error;
    } else {
      throw new ErrorResponse_js_1$3.InvalidArgumentException(`Coordinates or error should be set`);
    }
  ...
  async setLocaleOverride(params) {
    ...
    if (locale !== null && !isValidLocale(locale)) {
      throw new ErrorResponse_js_1$3.InvalidArgumentException(`Invalid locale "${locale}"`);
    }
    const browsingContexts = await __privateMethod(this, _EmulationProcessor_instances, getRelatedTopLevelBrowsingContexts_fn2).call(this, params.contexts, params.userContexts);
    for (const browsingContextId of params.contexts ?? []) {
      __privateGet(this, _contextConfigStorage2).updateBrowsingContextConfig(browsingContextId, {
  ...
  async setTimezoneOverride(params) {
    ...
    if (timezone !== null && !isValidTimezone(timezone)) {
      throw new ErrorResponse_js_1$3.InvalidArgumentException(`Invalid timezone "${timezone}"`);
    }
  ...
  async setUserAgentOverrideParams(params) {
    if (params.userAgent === "") {
      throw new ErrorResponse_js_1$3.UnsupportedOperationException("empty user agent string is not supported");
    }
  ...
  async setNetworkConditions(params) {
    ...
    if (params.networkConditions !== null && params.networkConditions.type !== "offline") {
      throw new ErrorResponse_js_1$3.UnsupportedOperationException(`Unsupported network conditions ${params.networkConditions.type}`);
    }
    await Promise.all(browsingContexts.map(async (context) => {
      const config = __privateGet(this, _contextConfigStorage2).getActiveConfig(context.id, context.userContext);
...
getRelatedTopLevelBrowsingContexts_fn2 = async function(browsingContextIds, userContextIds, allowGlobal = false) {
  if (browsingContextIds === void 0 && userContextIds === void 0) {
    ...
    throw new ErrorResponse_js_1$3.InvalidArgumentException("Either user contexts or browsing contexts must be provided");
  }
  if (browsingContextIds !== void 0 && userContextIds !== void 0) {
    throw new ErrorResponse_js_1$3.InvalidArgumentException("User contexts and browsing contexts are mutually exclusive");
  }
  const result = [];
  if (browsingContextIds === void 0) {
    if (userContextIds.length === 0) {
      throw new ErrorResponse_js_1$3.InvalidArgumentException("user context should be provided");
    }
    ...
    if (browsingContextIds.length === 0) {
      throw new ErrorResponse_js_1$3.InvalidArgumentException("browsing context should be provided");
    }
    for (const browsingContextId of browsingContextIds) {
      ...
      if (!browsingContext.isTopLevelContext()) {
        throw new ErrorResponse_js_1$3.InvalidArgumentException("The command is only supported on the top-level context");
      }
...
class BrowsingContextImpl {
  ...
  async setUserAgentAndAcceptLanguage(userAgent, acceptLanguage) {
    await Promise.all(__privateMethod(this, _BrowsingContextImpl_instances, getAllRelatedCdpTargets_fn).call(this).map(async (cdpTarget) => await cdpTarget.setUserAgentAndAcceptLanguage(userAgent, acceptLanguage)));
  }
...
class NetworkRequest {
  ...
  get bytesReceived() {
    var _a3;
    return ((_a3 = __privateGet(this, _response).info) == null ? void 0 : _a3.encodedDataLength) || 0;
  }
}
_id4 = new WeakMap();
...
setUserContextConfig_fn = async function(config) {
  ...
  if (config.viewport !== void 0 || config.devicePixelRatio !== void 0 || config.screenOrientation !== void 0) {
  ...
  if (config.userAgent !== void 0 || config.locale !== void 0) {
...
createCdpTarget_fn = function(targetCdpClient, parentCdpClient, targetInfo, userContext) {
  ...
  const target = CdpTarget_js_1.CdpTarget.create(targetInfo.targetId, targetCdpClient, __privateGet(this, _browserCdpClient10), parentCdpClient, __privateGet(this, _realmStorage6), __privateGet(this, _eventManager12), __privateGet(this, _preloadScriptStorage3), __privateGet(this, _browsingContextStorage14), __privateGet(this, _networkStorage4), __privateGet(this, _configStorage3), userContext, __privateGet(this, _logger13));
...
const _BidiServer = class _BidiServer extends EventEmitter_js_1.EventEmitter {
  constructor(bidiTransport, cdpConnection, browserCdpClient, selfTargetId, defaultUserContextId, parser, logger) {
    ...
    __privateSet(this, _commandProcessor, new CommandProcessor_js_1.CommandProcessor(cdpConnection, browserCdpClient, __privateGet(this, _eventManager14), __privateGet(this, _browsingContextStorage17), __privateGet(this, _realmStorage7), __privateGet(this, _preloadScriptStorage4), networkStorage, contextConfigStorage, __privateGet(this, _bluetoothProcessor3), userContextStorage, parser, async (options) => {
      ...
      new CdpTargetManager_js_1.CdpTargetManager(cdpConnection, browserCdpClient, selfTargetId, __privateGet(this, _eventManager14), __privateGet(this, _browsingContextStorage17), __privateGet(this, _realmStorage7), networkStorage, contextConfigStorage, __privateGet(this, _bluetoothProcessor3), __privateGet(this, _speculationProcessor2), __privateGet(this, _preloadScriptStorage4), defaultUserContextId, logger);
...
let BrowsingContext = (() => {
  var _navigation, _reason, _url2, _children2, _disposables2, _realms, _requests, _emulationState, _bluetoothEmulation, _deviceRequestPromptManager, _BrowsingContext_instances, initialize_fn7, session_get, createWindowRealm_fn, _a3;
  let _classSuper = EventEmitter$2;
  let _instanceExtraInitializers = [];
  let _dispose_decorators;
...
let BidiPage = (() => {
  var _trustedEmitter_accessor_storage, _browserContext, _frame6, _viewport, _workers5, _cdpEmulationManager, _emulatedNetworkConditions, _fileChooserDeferreds, _BidiPage_instances, initialize_fn7, _overrideNavigatorPropertiesPreloadScript, _requestInterception, _authInterception, toggleInterception_fn, applyNetworkConditions_fn, go_fn, _a3;
  ...
  return _a3 = class extends _classSuper {
    constructor(browserContext, browsingContext) {
       ...
       * @internal
       */
      __privateAdd(this, _overrideNavigatorPropertiesPreloadScript);
    ...
    async setUserAgent(userAgentOrOptions, userAgentMetadata) {
      ...
      await __privateGet(this, _frame6).browsingContext.setUserAgent(enable ? userAgent : null);
      const overrideNavigatorProperties = (platform2) => {
        if (platform2) {
      ...
      if (__privateGet(this, _overrideNavigatorPropertiesPreloadScript)) {
        await this.removeScriptToEvaluateOnNewDocument(__privateGet(this, _overrideNavigatorPropertiesPreloadScript));
      }
      const [evaluateToken] = await Promise.all([
        enable ? this.evaluateOnNewDocument(overrideNavigatorProperties, platform || void 0) : void 0,
        // When we disable the UserAgent we want to
        // evaluate the original value in all Browsing Contexts
        ...frames.map((frame) => {
          return frame.evaluate(overrideNavigatorProperties, platform || void 0);
        })
      ]);
      __privateSet(this, _overrideNavigatorPropertiesPreloadScript, evaluateToken == null ? void 0 : evaluateToken.identifier);
    }
  ...
  }, _overrideNavigatorPropertiesPreloadScript = new WeakMap(), _requestInterception = new WeakMap(), _authInterception = new WeakMap(), toggleInterception_fn = async function(phases, interception, expected) {
...
let BidiBrowser = (() => {
  ...
  return _a3 = class extends _classSuper {
    ...
    getWindowBounds(_windowId2) {
      throw new UnsupportedOperation();
    }
    setWindowBounds(_windowId2, _windowBounds) {
      throw new UnsupportedOperation();
    }
    targets() {
      return [
